...
function Invoke-DbQuery {
    ...
    try {
        ...
        while ($reader.Read()) {
            ...
            Write-Host "  $($cols -join ' | ')" -ForegroundColor White
            $rows++
        }
        $reader.Close()
        $conn.Close()
...
# Local BI databases with SC_UM tables
...
# Production - find all BI databases
Write-Host "`n--- Scanning production for BI databases ---" -ForegroundColor Magenta
